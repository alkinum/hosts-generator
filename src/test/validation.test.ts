...
describe('validateDomains', () => {
  describe('valid domains', () => {
    it('should accept valid single domain', () => {
      const result = validateDomains('example.com', mockT);
  ...
  describe('invalid domains', () => {
    it('should reject empty input', () => {
      const result = validateDomains('', mockT);
    ...
    it('should reject domains starting with numbers', () => {
      const result = validateDomains('123example.com', mockT);
      expect(result.domains).toEqual([]);
    ...
    it('should reject domains that are too long', () => {
      ...
      const result = validateDomains(longDomain, mockT);
      expect(result.domains).toEqual([]);
    ...
    it('should reject invalid IP addresses in hosts format', () => {
      const result = validateDomains('999.999.999.999 example.com', mockT);
      expect(result.domains).toEqual([]);
    ...
    it('should reject malformed hosts entries', () => {
      const result = validateDomains('127.0.0.1 example.com extra.part', mockT);
      expect(result.domains).toEqual([]);
    ...
    it('should reject single character domains', () => {
      const result = validateDomains('a', mockT);
      expect(result.domains).toEqual([]);
  ...
  describe('edge cases', () => {
    ...
    it('should handle tabs and multiple spaces', () => {
      const result = validateDomains('127.0.0.1\t\texample.com', mockT);
      expect(result.domains).toEqual(['example.com']);
      expect(result.errors).toEqual([]);
    });

    it('should handle minimum valid domain length', () => {
      const result = validateDomains('a.b', mockT);
    ...
    it('should handle maximum valid domain length', () => {
      ...
      const result = validateDomains(maxDomain, mockT);
